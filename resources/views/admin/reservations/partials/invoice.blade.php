            ...
            <div class="section">
                <div class="invoice-to">
                    @isset($booking->checked_in_at)
                        <h5 class="title">Checkin At: {{ $booking->checked_in_at }}</h5>
                    @endisset
                </div>
                <div class="billing-info">
                    @isset($booking->checked_out_at)
                        <h5 class="title">Checkout At: {{ $booking->checked_out_at }}</h5>
                    @endisset
                </div>
            </div>
            <h3 class="title">Room's Details</h3>
            <table>
                <thead>
                    <tr>
                        <th>Date</th>
                ...
                <tbody>
                    ...
                    @foreach ($activeBookedRooms as $key => $item)
                        @foreach ($item as $index => $booked)
                            <tr>
                                @if ($previousKey != $key)
                                    <td class="custom-table__subhead" rowspan="{{ count($item) }}" style="text-align: center;">
                                        {{ $key }}
                                    </td>
                                    @php
                                        $previousKey = $key;
                                    @endphp
                                @endif
                                <td class="text-start">{{ $booked->room->room_number }}</td>
                    ...
                    <tr class="custom-table__subhead">
                        <td class="text-end" colspan="3">Total Fare</td>
                        <td class="text-end">{{ number_format( $booking->booking_fare ,2) }}</td>
                    </tr>
                    ...
                    @if (!$extraService)
                        ...
                        @if ($due > 0)
                            <tr class="custom-table__subhead">
                                <td class="text-end" colspan="3">Due</td>
                                <td class="text-end">{{ number_format( $due ,2) }}</td>
                            </tr>
                        @elseif($due < 0)
                            <tr class="custom-table__subhead">
                                <td class="text-end" colspan="3">Refundable</td>
                                ...
                                <tbody>
                                    ...
                                    @if ($booking->cancellation_fee > 0)
                                        <tr>
                                            ...
                                            <td>{{ $booking->cancellation_fee }}</td>
                                        </tr>
                                    @endif
                                    <tr>
                                        <td class="text-end" colspan="3"> Tax</td>
                                        <td>{{ $booking->tax_charge }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-end" colspan="3">Service Charge</td>
                                        <td>{{ $booking->service_cost }}</td>
                                    </tr>
                                    @if ($booking->extraCharge() > 0)
                                        <tr>
