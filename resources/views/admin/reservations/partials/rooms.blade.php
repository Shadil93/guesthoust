...
<script>
    (function($) {
        ...
        let unitTax = roomInfo.room_type.tax;

        @if (@$requestUnitFare)
            unitFare = "{{ $requestUnitFare }}";
        @endif

        let roomNumber, length, checkInDate, checkOutDate, fare, amount, totalAmount, totalFare, taxCharge;
            ...
            if (!apendedRoom.length) {
                orderItemDiv.append(`
                    <li class="orderListItem order-list-type-${roomTypeId} list-group-item d-flex justify-content-between align-items-center room-${roomNumber}" data-room_number="${roomNumber}">
                        <input type="hidden" value="${roomId}-${date}" name="room[]" >
                        <span>
                            <span class="removeItem btn btn-sm btn-danger"><i class="ni ni-times"></i></span>
                            ${roomNumber}
                        </span>
                        <span class="totalDays">1</span>
                        <span class="unitFare">${parseFloat(unitFare).toFixed()} ${curText}</span>
                        <span class="unitTax" unit_tax="${parseFloat(unitTax).toFixed()}">${parseFloat(unitTax * 1).toFixed()} ${curText}</span>
                ...
                let tax = parseFloat(unitTax) * parseFloat(room.length); // Calculate tax based on number of days
                apendedRoom.find('.subTotal').attr('sub_total', subTotal).text(`${subTotal} ${curText}`);
                apendedRoom.find('.totalDays').text(room.length);
                apendedRoom.find('.unitTax').attr('unit_tax', tax).text(`${tax.toFixed()} ${curText}`); // Update displayed tax
            }
        ...
        function appendItemFirstTime(room) {
            ...
            $.each(room, function(index, singleRoom) {
                ...
                html +=
                    `<input type="hidden" value="${roomInfo.id}-${date}" name="room[]" >`;
            });
            ...
            appendHtml += `<span>
                            ...
                            <span class="unitFare">${parseFloat(unitFare).toFixed()} ${curText}</span>
                            <span class="unitTax" unit_tax="${parseFloat(tax).toFixed()}">${parseFloat(tax).toFixed()} ${curText}</span>
                            <span class="subTotal" sub_total="${parseFloat(fare).toFixed()}">${parseFloat(fare).toFixed()} ${curText}</span>
                        </li>`;
        ...
        function setTotalAmount() {
            ...
            let tax = $(document).find('.unitTax');
            $.each(tax, function(index, element) {
                taxCharge += parseFloat($(element).attr('unit_tax'));
            });
            ...
            $(document).find('.taxCharge').text(taxTotalCharge + ' ' + curText);
